...
const chapters = [
  {
    ...
    content: `Love has never belonged to one definition. It does not wait for permission. It does
     not arrive announced. It exists everywhere — in moments, in silence, in distance, in memory, in loss, in warmth. 
    We recognize it only when it touches us. And sometimes, even then, we fail to name it. There are loves that stay. There are loves that leave. There are loves that never speak aloud, yet change the course of a life. 
    Some begin with a glance. Some begin with a wound. 
    Some are built slowly — habit by habit — until one day they feel inevitable.
    
    
    
    This is not a story about love. This is a collection of the ways love appears.


This is not a story about one kind of love. This is not a tale of perfection. This is a collection of encounters — of two hands that met at the right time, of two souls that met at the wrong one, of the quiet devotion no one applauds, of the kind of love that hurts, but refuses to disappear. 
Every chapter carries a different face. Every page carries a different weight. Some will feel familiar. Some will feel uncomfortable.
 All of them are real. Because love is not rare. What is rare is recognizing it while it is still in your hands. 
...
export default function StoryPage() {
  ...
  const switchChapter = (slug: string) => {
    ...
    setTimeout(() => {
      setActiveSlug(slug);
      router.push(`/story?chapter=${slug}`, { scroll: false });
  ...
  return (
    <main className="relative min-h-[200vh] text-white">
      ...
      <section className="pt-32 pb-24 px-8">
        ...
        >
          {/* TITLE */}
          <h1 className="text-5xl md:text-6xl font-bold text-[#3A2416] transition-opacity duration-500">
            {activeChapter.title}
          </h1>

          {/* FIRST 3 PARAGRAPHS — animated feel */}
          ...
          {/* REST OF STORY — snap replace */}
          {paragraphs.slice(3).map((para, index) => (
            <p
              key={index}
              className="text-[#3E2F23] text-lg leading-relaxed whitespace-pre-line"
            >
              {para}
            </p>
          ))}
